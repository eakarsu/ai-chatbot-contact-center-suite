const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:5203';
...
const adminCookie = await login('admin@ai-contact-center.local', 'admin123');
const managerCookie = await login('manager@ai-contact-center.local', 'manager123');
const analystCookie = await login('analyst@ai-contact-center.local', 'analyst123');

await expectJson('/api/dashboard', adminCookie);
await expectJson('/api/entities/chatbots', analystCookie);
...
const records = await expectJson('/api/entities/chatbots', managerCookie);
const rowId = records.rows[0].id;

const approveResponse = await fetch(`${baseUrl}/api/entities/chatbots/approve`, {
  method: 'POST',
  headers: {
...
const forbiddenApprove = await fetch(`${baseUrl}/api/entities/chatbots/approve`, {
  method: 'POST',
  headers: {
...
console.log('AI Chatbot Contact Center Suite smoke passed');
